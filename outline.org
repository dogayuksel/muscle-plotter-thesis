...
* Introduction
  ...
  The main strength of EMS is that the resulting systems miniaturize well, thus lend themselves well to mobile use (mobile gaming \cite{lopes_musclepropell}) or wearable use (pedestrian cruise control \cite{pfeiffer_cruise}). A second key strength is their ability to implement input/output interactions that use the same modality (aka symmetric interaction \cite{roudaut_gesture}) by using the same gesture language for input and output (\cite{lopes_proprioceptive}).

  Unfortunately, the price for these benefits is that the interactive EMS systems presented so far lack expressiveness. Existing interactive EMS systems output a single 1D output variable, such as screen tilt \cite{lopes_musclepropell} or wrist tilt \cite{lopes_proprioceptive} or one of n behaviors \cite{lopes_affordance++}. Since subsequent output overwrites earlier output, users can only see a single value at a given time.
...
** Contribution
   ...
   \App{} extends on this line of work by providing a more expressive EMS-based system, which is attained by persisting the EMS output onto the paper. Older systems communicate via discrete poses of the users’ body, resulting with a single value any given time. User's wrist is actuated while dragging the pen to lay down a curve. This interaction lends itself to more expressive and longer interactions with ability to communicate series of values.
...
*** Current user study verifies the accuracy but not the usability.
...
*** Interactive Text Books
    \app{} interactions require some input from the user side, in order to determine the domain and configure the output. In the simplest case, user has to acomplish a sequence of inputs to get a the first output. Predesigned worksheets would allow to simplify these steps, minimizing the learning curve as well as creating engaging interactions. An example of a worksheet could include interactive math textbooks.
